...
- **GitHub Actions diagnosis (reproduced locally — gh CLI unauthenticated, so the CI steps were replayed against a throwaway PG):**
  1. **`Deploy to Hetzner` (deploy.yml)**: fails on EVERY push by design flaw — it auto-triggers on `push: main` and SSHes to a Hetzner server that is **not provisioned yet** (v1 Streamlit pipeline, now retired to legacy/). Fixed: **manual-only** (`workflow_dispatch`), documented; the v2 cutover path stays deploy-v2.yml.
  2. **`Postgres dual-CI`**: dual_ci ✅ and parity ✅ replayed green — the failure is the **NL→SQL round-trip check** (landed with cutover-day Phase C) connecting as the true read-only role **`gi_ai_ro`, which nothing ever creates in CI's fresh database** (locally the runbook re-runs `create_ai_readonly_role.sql` after every reload). Fixed: new workflow step provisions the role + password and the service_tests step gets `GI_AI_RO_URL`. **Repro verified: 744/0 → after provisioning → PASS on the CI-style throwaway DB.**
  3. **`deploy-v2.yml` gate**: still referenced the pre-restructure `backend/dual_ci.py` + `backend/api/parity_check.py` — re-pathed to `tools/` + same RO-role step added. (Docker smoke-build not verifiable locally — flagged for the first manual run.)
- **Bug Tracking Engine** (extends the existing `bug_reports` feedback system — no parallel tables): alembic **`c7d4e8f19a25`** adds `title/severity/rollback_notes/safety_constraints/triage_notes`; submission takes title+severity (validated); admin PATCH takes the triage trio; **`GET /admin/feedback/{id}/prompt`** renders a SELF-CONTAINED implementation prompt (report + triage analysis + safety constraints + rollback plan + the project's non-negotiable gates) and **`GET /admin/feedback-export.md`** bundles a status-filtered digest. UI: FeedbackPage gains title+urgency; Admin Console feedback tab became a triage drawer + **📋 Prompt copy** + digest download. **The automation loop:** report → admin triages (constraints/rollback captured) → copy the prompt → paste into a Claude Code session on the repo → the agent implements under the stated gates → status back to resolved. The portal itself never mutates code — stability by construction.
- **Gates:** service_tests **750/0** (new suite AO, 6 checks) · bug_check 599/0 (allowlist +5 new-stack columns) · Playwright 39/39 · build+tsc ✅ · alembic single head **c7d4e8f19a25**.

### 2026-07-18 (PRE-DEPLOY batch · Phase 4) · actor=interactive · branch=`main` · 🧊 Frozen table headers · 🔢 smart decimals · 📑 report column scoping
- **Sticky headers**: scripted sweep added `sticky={{ offsetHeader: 64 }}` (the app-header height; BrowseTable precedent) to **93 antd Tables across 42 files** — brace/quote-aware opening-tag scanner, existing sticky props skipped. SmePage's five master grids + the Smart Calculator use the **live-measured band offset** (`headH + 46`) since its pinned title+tabs replace the app header; the other SME tab components keep 64 (their headers tuck under the taller band while pinned — no worse than the previous non-sticky behaviour, noted as minor polish).
- **Smart decimals**: new `lib/format.ts` (`fmtQty`/`fmtCell` — whole numbers render clean, fractions keep ≤4 dp, numeric-looking strings from raw SQL rows included) wired into the generic `buildColumns` cell renderer; SME coverage **percentages deliberately keep** their fixed `.1f` style (and `engine.ts` status strings are golden-parity-pinned — untouched).
- **Report scoping** (backend `reports.py`, so xlsx/pdf/csv all inherit): **Current Stock** no longer `SELECT *`s the ledger totals (identity + Current_Stock + Minimum_Qty only — received/consumed/returned belong to their own reports); **Consumption / Daily Consumption / Goods Receipts / WBS / Burn Rate** gained the **Material description** via the inventory join (where-clauses re-qualified — `Site_ID` would have been ambiguous). Monthly Summary keeps Opening/Received/Issued/Returned/Closing — that IS its purpose.
- **Gates:** service_tests **744/0** (AN +2 report-scoping pins) · Playwright 39/39 · build+tsc ✅ · preview: `.ant-table-sticky-holder` active on the swept tables (full interactive check limited by the known preview hidden-tab throttling).

### 2026-07-18 (PRE-DEPLOY batch · Phase 3) · actor=interactive · branch=`main` · 🛡️ Surface-Shields issue workflow + 🧮 SME Smart Calculator
- **`GET /entry/lining-systems`** (any authenticated, site-resolved): recipe lines grouped per system code (short name, substrate, materials w/ For_1_SQM, the recipe's SAP list via `sme_recipe.SAP_Code`) + per-code SQM progress (equipment ⟕ progress; done = Done+staged, pending = max(orig−done,0); falls back to Surface_Area_SQM like the engine's load_all) + a `sap_index` reverse map. Powers BOTH new features.
- **IssuePage workflow (enforced)**: Surface Shields context (category filter or a picked shield SAP) surfaces a **Lining System selector FIRST** — the material picker filters to that recipe's SAPs; **Done/Pending/Original SQM + unit count** tags render live; `addToBatch` **refuses** a Surface-Shields material without a selected system, and the code travels to the HOD as a `LS <code> (<short>)` Remarks suffix (no consumption schema change — deliberate).
- **`GET /sme/calculator?code&sqm`** (level ≥2; SmePage 🧮 Smart Calculator tab): recipe demand math (`For_1_SQM × SQM`, the CNCEC RL/BL prediction-project model), per-line `required_qty`, pack counts from Package_Size, **live ERP stock through the SAP join** with per-line shortfall + a human explanation string ("2.5 KG/SQM × 40 SQM = 100 KG → 4 × 25 KG pack(s) · in stock: 150 ✓"); guards: 404 unknown code, 422 non-positive SQM.
- **Gates:** service_tests **742/0** (new suite AN, 5 checks) · tsc ✅.

### 2026-07-18 (PRE-DEPLOY batch · Phase 2) · actor=interactive · branch=`main` · 🤖 Ask-data deep filters + handwritten-OCR spec implementation
- **Ask-data**: template lane learned **category filters** (bound `ILIKE :cat`; R/L families always fire, generic categories need the word "category") and **material keywords** ("remafix", "furan", "chemoline"… → `ILIKE` over description/material-code **plus the SME tables via the new SAP join** — `EXISTS sme_recipe sr WHERE sr."SAP_Code"=i."SAP_Code"`, seed via padded-list match). Message strings show the applied filters. NL lane: SCHEMA_HINT gained sme_recipe/sme_equipment/sme_sqm_progress schemas, the SAP-join fact, a "never answer material families from memory — filter with ILIKE" rule + a furan worked example.
- **Handwritten OCR (docs/features/handwritten-ocr v1.0 — spec committed in-repo)**: new `backend/api/ai/handwritten.py` implements every deterministic stage exactly: 3-format date parsing with digit fixes + year gates · 18-entry corrections list · ditto-glyph resolution · qty rules (additive sums, blank→1, zero rejects, ~approx) · the spec fuzzy scorer (conf≥40 + lead≥8 auto-accept, top-5 candidates) · the CLOSED 8-rule substitution table (R1–R4) · whole-batch stock simulation ((date,form,row) order, low-stock 5, negative→blocked) · [?]/⚠️/🚨 flag taxonomy · row_id format · **17-column legacy TSV export** (blocked rows never exported). `CONSUMPTION_PROMPT` rewritten for faithful transcription of the MPC3P1-CNCEC form (ditto glyphs verbatim, struck-through reported not dropped, qty_text raw, top-right date). New SK-exact-locked `POST /ai/ocr/handwritten-process` (read-only; posting stays in the Issue flow); OcrImportPage gained **Validate (handwritten spec)** + **TSV export** with flag markers in the grid.
- **Gates:** service_tests **737/0** (new suite AM, 10 checks) · build+tsc ✅.

### 2026-07-18 (PRE-DEPLOY batch · Phase 1) · actor=interactive · branch=`main` · 🔗 SME SAP-code overhaul + Excel re-sync (SYSTEM CODES RENUMBERED)
- **Workbook changes:** For_1_SQM + Materials gained a **SAP_Code column** (exact ERP↔SME join keys); recipe sheet deduped 103→54 rows; **Lining_System_Codes RENUMBERED** (old 1=CBL30 → new 4; old 7=RLCB4 → new 1 & 3; the composite systems 77/88/333/444/777/888 are GONE — replaced by per-area base-code equipment rows, Equipment.xlsx 263→315). Inventory sheet now carries the variant SAPs (1040-1, 1041-1/-2/-3 …) as real items — they are the PU Comp-A/B/C/D component identities.
- **Alembic `b3f2a9c47d18`**: `sme_recipe.SAP_Code` + `sme_inventory_seed.SAP_Code` (TEXT NULL); recipe UNIQUE widens (code, material) → **(code, material, SAP_Code)** — PU systems have 4 component lines per material distinguishable only by variant SAP. models.py matched; bug_check models-parity allowlist extended (new-stack-only columns).
- **bulk_import**: `plan_sme_recipes` line identity is now (code, mat, SAP); SAP-aware files **merge repeated identities as coat lines (For_1_SQM sums** — CONDL2 primer+body coat); legacy no-SAP files keep first-occurrence-wins. `plan_sme_materials` records the distinct variant-SAP list per material (comma-joined). `sme_master` recipe CRUD accepts SAP_Code + dup-checks the triple.
- **excel_sync**: new `--kinds` filter + **`--sme-reseed`** (drops recipes global / equipment+progress per-site / seeds before the SME loads — REQUIRED for a code renumbering, where upserts would leave stale old-code rows double-counting SQM; aborts if any Done_SQM>0 would be lost unless `--force-drop-progress`; verified 0/102 progress rows carry Done_SQM).
- **EXECUTED (additive half):** `--kinds inventory,ledger --commit` → inventory +6 variant-SAP items / 33× Surface-Shield canonicalisation · receipts +18 (+5 corrections) · consumption +40 · returns +1; reconcile zeroed 4 superseded rows (incl. return 1239 whose date-less line got a real date in the workbook) → **STOCK VERIFICATION: 429/429**. ⚠️ **The SME reseed commit is OPERATOR-RUN** (deletes are classifier-blocked in auto mode): `.venv/bin/python tools/excel_sync.py --site CNCEC --kinds sme-equipment,sme-recipes,sme-materials --sme-reseed --commit` — dry-run verified (+66 equipment / +41 recipes / 22 seed updates after dropping 102+77+86+22 stale rows).
- **Also:** re-applied `backend/scripts/create_ai_readonly_role.sql` (users-REVOKE had drifted — the known post-DDL recipe).
- **Gates:** service_tests **727/0** (AJ +3: variant-SAP lines apart · coat-line sum · seed SAP list) · bug_check 599/0 · Playwright 39/39.

### 2026-07-14 (PRE-DEPLOY · workbook restructure) · actor=interactive · branch=`main` · 🧭 Header-name-driven Excel sync — operator reordered/added/deleted workbook columns
- **Trigger:** the operator restructured the four tracking workbooks (now living at the **repo root**, no longer `~/Downloads`) ahead of the final local cutover test — column reorders + new columns in `CNCEC_Inventory.xlsx` (`Current Location`, `Audit 13/06/26` on Inventory; `Cons. Paper No.` + `Pallet No.` on Consumption Log). Nothing the importers require was deleted.
- **`backend/api/bulk_import.py`**: every sheet already resolved columns by header name → reorders were free. Added **"ignored unmapped column(s)" warnings** (inventory + all 3 ledger sheets, with per-spec intentional-ignore lists) so a future restructure can never drop data silently; newly mapped the columns that DO have DB homes — `Remarks` on all 3 ledger sheets, `DN. Copy` → `receipts.DN_Copy`. `Current Location` has **no master home by design** (bins live per-receipt as `Bin_Location`, latest-wins) and `consumption` has no Pallet/paper-number columns — all ignored with the new warning/spec notes.
- **`tools/excel_sync.py`**: the stock-verification tail read SAP/Current-Stock **positionally** (`r[1]`/`r[10]`) — the one real breakage risk; now header-driven via `bi._sheet_rows`/`bi._col`. Default `--dir` → repo root. **`tools/excel_sync_reconcile.py`**: same fix (Inventory + all 3 log sheets were positional, incl. hardcoded ref-column indices); default `--workbook` → repo root.
- **Live dry-runs against the mirror with the restructured workbooks:** `excel_sync.py` → inventory =423 unchanged / ledger fully matched (only the 4 known date-less rejects) / **`STOCK VERIFICATION: 423/423`**; `excel_sync_reconcile.py` → 0 mismatched SAPs, 4 date-less lines correctly skipped as already-inserted. The operator's edits were purely structural — no data drift.
- **Gates:** service_tests **724/0** (suite AJ +3: reordered-header mapping, Remarks/DN_Copy capture + unknown-column warning, spec-ignored `Cons. Paper No.`) · Playwright 39/39 (one offline-queue flake on first run, clean on rerun) · no frontend changes.

### 2026-07-13 (PRE-DEPLOY · Phases 2–4) · actor=interactive · branch=`main` · 🤖 OCR doc assist (C3) + LLM tuning · 🏷️ QR/returnables parity · 📖 role-based user manual
- **C3 OCR doc assist (`7ac63c2`)**: `POST /ai/jobs/from-attachment` (SK exact-lock, `_require_ocr` flag gate) re-reads an already-uploaded `entry_attachments` BLOB through the existing vision pipeline; `EntryDocsUpload` gained a per-attachment **⚡ Read (AI)** button (2 s polling, 90 s cap, friendly degradation). ReceivePage auto-fills Date/Supplier and carries the DN header (`DN_No`/`Vehicle_No`/`Driver_Name`/`Mob_From`/`Prepared_by`) into every staged row via `ReceiptIn.extra`; IssuePage pre-fills SAP/qty/Issued_To from a single auto-matched row. **Ask-data validation**: SCHEMA_HINT taught the post-injection schema (Opening_Stock always-0 rule, DN_No/WBS/Tank_No/Reason, the Category vocabulary incl. MTC-gated 'Surface Shields' with ILIKE-singular guidance, site codes) + 2 new worked examples; suite AK pins the hint content so it can't silently regress. Ollama was offline → template lane validated live against the injected data (suite AD recomputes DB truth), NL lane validated structurally + via the existing stubbed round-trips.
- **QR + returnables parity (`643e363`)**, closing the recon audit's actionable gaps: single-employee **badge PNG** (`GET /documents/employee-badge/{id}`, site-scoped); label sheets honor **repeated sap_codes** (per-item copies) + DocumentsPage material multiselect/copies UI; ReturnablesPage return-side **"Employee QR check"** (scan a badge → filter to that borrower's open loans); `create_returnable` now writes the **Smart-Scan adoption audit** (`cv_detected/cv_employee_id/cv_tool_class/cv_confidence` — columns existed since legacy, never set by v2). Left as documented decisions: the SK→HOD QR-label request/approval workflow (optional follow-up) and the trainable CV tool-detection model — the 2026-07-06 ruling stands (qwen2.5vl names tools; populate `tool_catalogue` to constrain classes; a YOLO train→promote lifecycle remains possible post-deploy if the operator wants confidence-gated verification).
- **Role-based user manual**: new `docs/USER_MANUAL.md` (v2.0) — per-role chapters (SK/HOD/Logistics/Warehouse/Supervisor/Admin), PWA install + offline queue, document/MTC/WBS/return gates, OCR doc assist, ask-data, WhatsApp commands, bulk-import format rules, troubleshooting — with **8 live screenshots** captured from the running stack via `tests/e2e/capture-manual-shots.mjs` into `docs/screenshots/v2/`. The in-app manual/PDF pipeline still serves the legacy root manual until repointed post-deploy (noted in the doc).
- **Gates:** service_tests **721/0** (suites A…AL: +21 AJ, +13 AK, +6 AL) · Playwright 39/39 · build+tsc ✅.

### 2026-07-13 (PRE-DEPLOY · Phase 1A) · actor=interactive · branch=`main` · 📊 One-time Excel data injection — EXECUTED, stock verified 423/423
- **Ran `tools/excel_sync.py --commit`** (through the same audited service code as `/import/*`) against the :5433 mirror, site CNCEC, from the operator's four updated workbooks: **inventory** +130 new SAPs / ~239 updated / 0 rejected (28 × `"Surface Shield" → "Surface Shields"` canonicalisation — all 28 confirmed MTC-gated; 12 Material_Code uniqueness conflicts imported codeless with warnings — the workbook's variant SAPs `1041-2/-3/-4` etc. share codes the DB's UNIQUE constraint can't hold) · **ledger backfill** +414 receipts / +379 consumption / +8 returns, 4 qty-corrections (the workbook zeroed mis-entries, e.g. SAP 1010's 04-28/04-29 receipts), 57 exact multiset matches skipped · **sme-equipment** ~69 re-baselined (Done_SQM preserved) · **sme-recipes** ~86 · **sme-materials** ~5.
- **`tools/excel_sync_reconcile.py --commit`** (step 2): zeroed **16 DB-only ledger rows** on the 17 stock-mismatched SAPs — all app test artifacts (literal `asdf` rows, the long-flagged junk consumption id=1, 2.01/1.1/2.1-style UAT quantities) or double-entries (same DN logged on adjacent days) — Quantity→0 + explanatory Remarks, **never deleted**; inserted the **4 half-entered workbook lines** that had no Date cell (stamped 2026-07-13 + `[excel-sync]` remark). One `BULK_IMPORT_RECONCILE` audit row.
- **Verification: `STOCK VERIFICATION: 423/423`** — every SAP's DB stock (Opening + Σreceipts − Σconsumption − Σreturns) equals the workbook's Current Stock column exactly. DB now: inventory **436** · receipts **487** · consumption **380** · returns **11**.
- **⚠️ CONSEQUENCE (documented in ARCHITECTURE §1 + the cutover runbook):** PostgreSQL is now permanently AHEAD of the frozen `gi_database.db`. Any `dual_ci`/cutover reload wipes the injection → re-run both sync tools after EVERY reload (incl. on the production box after the final load). `tools/parity_check.py` therefore fails against the live mirror **by design** — it stays meaningful only on CI (self-loaded PG) or a fresh mirror. Snapshots: pre-sync + post-sync `pg_dump -Fc` (post: `~/Downloads/gihub_post_excel_sync_20260713.dump`).
- **Post-injection gates:** service_tests **702/0** on the enriched mirror (all suites are data-robust) · Playwright 39/39 (own DB, unaffected).

### 2026-07-13 (PRE-DEPLOY · Phase 1B) · actor=interactive · branch=`main` · 📥 Bulk Excel Import — the operator's "update from Excel" feature
- **Backend `backend/api/bulk_import.py`** (`POST /import/{kind}`, dry-run default + `?commit=true`): five kinds — **inventory** (master upsert on SAP; aggregate columns ignored — stock stays ledger-derived; category canonicalisation `"Surface Shield"→"Surface Shields"` so the MTC hard-block keeps matching; Material_Code uniqueness resolved file-wide: in-file dup → first-wins/codeless, owner re-mapped in the same file → release+reassign, owner untouched → import codeless with a warning), **ledger** (append-only backfill of the workbook's Receipt/Consumption/Return Log sheets with a three-tier reconcile: exact multiset match → skip · same (day, SAP, ref) different qty → UPDATE correction · else INSERT; orphan-SAP rows rejected; DB-only rows never deleted, only reported), **sme-equipment** (legacy `_clean_equipment` port: Name-identity tag backfill, short-name→code map, non-numeric code skip, per-(tag,code) area-SQM summing, Location canon; progress re-seeded PRESERVING Done_SQM), **sme-recipes** (comma-pair Material_Code cells split into one line per material, in-file repeats first-wins — matches the legacy bootstrap's INSERT-OR-IGNORE outcome), **sme-materials** (`_clean_inventory_seed` port: PO lines aggregated per code, latest Document_Date wins). Roles: SME kinds {hod, admin} (HOD site-pinned), inventory/ledger admin-only. 8 MB cap, xlsx magic-byte check, every commit audited (`BULK_IMPORT_*`).
- **Frontend `BulkImportPage`** (`/bulk-import`, HOD-group nav): kind picker (admin-only kinds hidden for HOD), site picker for unscoped users, drag-drop upload → **Dry-run** (summary/warnings/rejects tables) → **Commit** (Popconfirm; disabled until a dry-run ran).
- **`tools/excel_sync.py`** — the one-shot sync runner: drives the SAME plan/apply functions in the safe order (inventory → ledger → SME kinds), prints each plan, and ends with a per-SAP **stock verification** (DB Opening+Σledger vs the workbook's Current Stock column). Reusable verbatim on the production box after the final cutover load.
- **Tests: suite AJ** (+21 → **702/0**): role locks, unknown-kind 404, non-xlsx 422, dry-run writes nothing, category canonicalisation + the imported item IS MTC-gated (live `/entry/receipt-meta` check), field-diff updates, ledger insert/orphan-reject/zero-skip → idempotent re-import → qty-correction 5→3 with no dup row, equipment area aggregation + Location canon + Name-identity + progress seed + Done_SQM preservation across re-baseline, recipe comma-split first-wins, materials PO aggregation, audit actions. Hermetic (in-memory workbooks, SVCJ- cleanup).
- **Gates:** service_tests **702/0** · Playwright 39/39 · parity 5/5 · build+tsc ✅.

### 2026-07-13 (CUTOVER DAY · PHASE B) · actor=interactive · branch=`main` · 📦 Repository restructure — root becomes the new stack's home
- **The pre-approved one-commit restructure executed** (REPO_MAP plan): **`legacy/`** ← the complete Streamlit app (`main.py`, `database.py`, `auth.py`, `config.py`, `ui_components.py`, `cache_layer.py`, `error_handling.py`, `mailer.py`, `reports.py`, `recover.py`, `migrate_role.py`, `whatsapp_worker.py`, `pages_internal/`, `services/`, `ai/`, `pwa/`, `scripts/` (bootstrap/ops), `tests/` (pytest), `.streamlit/`, gates `bug_check.py`/`test_ui_crawler.py` + reports, legacy deploy surface `docker-compose.yml`/`Dockerfile.streamlit`/`Dockerfile.fastapi`/`docker/`/`host_setup/`, `packages.txt`, `requirements-server.txt`, `pytest.ini`, runtime `uploads/`/`logs/`/`backups/`) · **`data-archive/`** ← root artifacts (seed xlsx ×3, sample PO pdf, `IMG_2397.JPG`, `gi_database.*.bak`, `PyWhatKit_DB.txt`, `demo_seed.db`) · **`tools/`** ← the bridge (`dual_ci.py`, `migrate_sqlite_to_postgres.py`, `parity_check.py` (ex backend/api), `pg_smoke.py`, `migration/` = cutover script + runbook).
- **Deliberate root keepers:** `gi_database.db` (live legacy data + the source for the final production `cutover_migrate.py` load; never staged), `reports_archive/` (shared runtime), SOP/manual markdown+PDF+builders (SERVED by `backend/api/documents.py` and read by `ai/manual_qa.py`), Meta compliance HTML (registered by URL), shared `requirements.txt`, `handoff.md`, `run_api.sh`.
- **Reference fixes:** bridge-tool `sys.path` blocks re-rooted (dual_ci → +`legacy/` for `import database`; migrate → +`backend/` for models; cutover_migrate → +`tools/` for its siblings; parity_check root calc); **`backend/api/sme_export_layouts.py` logo vendored** to `backend/api/sme_logo.png` (was reaching into `pages_internal/` — the new stack no longer touches `legacy/`); Playwright `global-setup.ts` → `tools/migration/cutover_migrate.py`; CI workflow paths+steps (`legacy/bug_check.py`, `tools/dual_ci.py`, `tools/parity_check.py`, trigger paths `legacy/**`+`tools/**`); runbook command paths; REPO_MAP rewritten for the new layout; ARCHITECTURE/PROJECT_STATUS commands updated.
- **bug_check repairs (path-driven + one latent):** root-asset checks re-pointed via `REPO_ROOT.parent` (shared `requirements.txt` ×2, `USER_MANUAL.md`, `docs/screenshots/`), root on `sys.path` for `build_manual_pdf` imports, bridge-tool spec-loads → `tools/`. Plus a **pre-existing latent failure surfaced by actually re-running the gate**: the models↔fresh-schema parity check never learned the UAT-5 cutover-audit rulings — `consumption`/`receipts` `wbs` (fresh, lowercase) vs `"WBS"` (real DB casing preserved in models.py) now equated, and the user-authorized preservation set (`inventory.Sl_No`, `consumption.status/Technician`, `pending_issues.Technician`, `rejected_issues_archive.Technician`) allow-listed with the audit citation.
- **Gates after the move:** `legacy/bug_check.py` **599/0** · `legacy/test_ui_crawler.py` **21/21** · service_tests **681/0** · `tools/parity_check.py` **5/5** · parity:sme 509 · build+tsc ✅ · Playwright **39/39** (throwaway DB loaded via the moved cutover script) · alembic single head `e7c31a9f24d5`. To run legacy locally: `GI_DB_FILE=../gi_database.db` from `legacy/`.

### 2026-07-13 (CUTOVER DAY · SME S6) · actor=interactive · branch=`main` · 🗄️ SME Master Data CRUD — the last parked phase ships
- **Why now:** the sme_* write freeze existed solely to prevent dual-write drift while legacy SQLite was the system of record; with the cutover migration executed, the new stack is the sole writer, so the locked ruling's own unlock condition is met.
- **Backend `backend/api/sme_master.py`** (router `/sme/master`, exact-lock `require_roles("hod")` — the legacy PAGE_ACCESS `{hod, admin}`): full port of the legacy Tab 8 semantics from `database.py` §R20.5 — **equipment** (site-scoped; create = one row per (tag, code) + `sme_sqm_progress` upsert seeding `Original_SQM` while PRESERVING `Done_SQM`; update deliberately does NOT cascade progress — legacy cell-edit; delete cascades the progress row; dup (site,tag,code) → 409), **recipes** (global; unique (code, material) → 409), **materials** (writes `sme_inventory_seed` ONLY per Canon Rule 2 — POST is the legacy INSERT-OR-REPLACE upsert on `Material_Code`, PATCH never renames the PK, ERP `inventory` untouched), **progress** (None-preserving upsert; standalone scopes allowed), **settings** (locations/types in `system_settings`, site-scoped, duplicate → 409, delete refused while equipment at the site uses the value). HOD writes pinned to their own site (foreign `site_id` → 403); admins must name a site (422). **Every write audits** to `system_audit_log` (`SME_CREATE_EQUIPMENT` … `SME_DELETE_SETTING`) — an upgrade over legacy, which audited nothing here. Reads keep explicit-PK ordering (Canon Rule 1).
- **Frontend:** new `frontend/src/sme/MasterData.tsx` — 🗄️ Master Data tab in SmePage (hidden below {hod, admin}) with Equipment / Recipes / Materials-seed / SQM-Progress / Locations-&-Types sub-tabs; antd Table + modal-form + Popconfirm pattern (MasterDataPage lineage); admin without a site sees an info alert with writes disabled (HODs are pinned server-side); every mutation invalidates the whole `/sme` query family so the Dashboard/Session/coverage tabs recompute instantly. Hooks in `api/hooks.ts` (`useSmeMasterList/Create/Patch/Delete`, `useSmeProgressUpsert`, settings hooks).
- **Engines untouched:** CRUD changes model INPUTS, never math — `sme_parity_fixture.json`/`sme_parity_golden.json` unchanged, `parity:sme` **509 PASS**.
- **Tests: suite AI** (+32 → **681/0**): role locks (worker/supervisor 403), create+progress-seed+audit, dup 409s, foreign-site 403, admin-no-site 422, model-snapshot pickup, edit-without-progress-cascade, None-preserving progress upsert, settings add/dup/in-use-guard/delete, delete-cascade, recipe/material CRUD incl. upsert re-baseline + ERP-inventory-untouched cross-check; the suite commits real rows with full SVC6- cleanup (audit rows stay — append-only contract). Logins use their own X-Real-IP (the shared bucket is long burned by suite AI's slot in the run order).
- **Verified live in the browser** (Vite :5173 → dev API :8000 on the migrated mirror): Master Data tab renders for hod+admin, admin site-guard alert + disabled Add, HOD add-equipment modal → **POST 201** → row `CNCEC/UI6-TANK-1/9902` + progress 25/0 + `SME_CREATE_EQUIPMENT` audit in PG → grid auto-refetch; test row removed after.
- **Gates:** service_tests **681/0** (A…AI) · parity_check 5/5 · parity:sme 509 · build+tsc ✅ · Playwright **39/39** · alembic head unchanged `e7c31a9f24d5` (no schema change).
