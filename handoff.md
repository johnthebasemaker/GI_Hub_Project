...
**Last update:** 2026-07-04 — **Operational ERP + estimator core is feature-complete on the new FastAPI+PostgreSQL+React stack; procurement runs end-to-end, an admin console manages users + audits, and a notification bell lights up the workflow.** Built + verified: auth/roles · staged data-entry · HOD approvals/burn-rate · **full procurement loop** (**HOD creates PR** → submit → Logistics PO → assign → Warehouse receive → DN → ship → site receive → HOD approve → ledger) · Supervisor requests → SK · SME estimator (read-only) · **Admin console** (user management + audit-log viewer) · **in-app notifications** (sidebar bell, role/site/warehouse/user-scoped, wired to 5 procurement events). This session added the **PR-creation UI**, the **Admin console** (users · audit · inventory Master-DB editor), the **notification bell**, **2FA self-enrollment**, **Reports** (Excel/PDF/CSV export), a **hardening pass** (service+guard tests gated in CI · master-data write role-gate), **cutover readiness** (JWT_SECRET fail-fast in production · code-split bundle 1.3 MB→288 kB), and **user registration + approval** (self-service Request-Access → admin approval) — the new stack is now feature-rich, self-sufficient, and ship-ready (only the deploy + make-React-primary decision remains). Streamlit/SQLite untouched. Also landed: a **turnkey deploy kit** (`deploy/` + `docs/DEPLOY.md`, not run) and the four **Tier-1 review quick wins** (Alembic migrations · CI frontend build · React error boundary · rate-limited public auth). Tests: **599 bug_check / 0 · UI crawler 21/21 · derived-view parity PASS · dual_ci 64/64 · service+guard tests 54/0 · frontend `npm run build` green**. Per-item detail in commit messages (`git log --oneline`).

> ### 📍 WHERE WE ARE — read this first
>
> **🆕 CONTINUING THE NEW REACT/FASTAPI/POSTGRES BUILD? → START WITH [`docs/NEW_STACK_HANDOFF.md`](docs/NEW_STACK_HANDOFF.md).** It is the self-contained fresh-chat entry point: its top **🎯 CURRENT STATUS** banner says the build is **feature-complete + ship-ready** (only cutover remains — the user's call), plus how to run it, login creds, golden rules (don't touch `database.py`; SME frozen; keep local PG == SQLite), the full DONE list (§3), and the OPTIONAL improvement backlog (§5). **Deploy runbook: [`docs/DEPLOY.md`](docs/DEPLOY.md).** Per-slice detail is in `docs/POSTGRES_MIGRATION.md` §8. Still Streamlit-only (not ported): WhatsApp, email, local-LLM/OCR, CV.
>
...
>
> ### 🧭 STATUS (updated 2026-07-04) — the new-stack BUILD IS COMPLETE
> - **DONE (all committed + pushed, verified on real PG, SQLite/Streamlit untouched):** the full operational + estimator core **plus** everything that was on the "WANT" list. Ledger services (receive/issue-FEFO/return/adjust) → staging → HOD approval · auth (JWT/bcrypt/TOTP/role-gated nav) · all portals (HOD · Logistics PR→PO→assign · Warehouse receive→DN→ship · Site receiving · Supervisor→SK) · **SME estimator (read-only)** · **PR-creation UI** · **Admin console** (users · audit viewer · inventory Master-DB editor) · **notification bell** (procurement + staging→HOD + approve/reject→submitter) · **2FA self-enrollment** · **Reports** (Excel/PDF/CSV) · **user registration + approval** · **hardening** (52-check service+guard suite gated in CI · master-data write role-gate) · **cutover readiness** (JWT_SECRET fail-fast in prod · code-split bundle) · **turnkey deploy kit** (`deploy/` + `docs/DEPLOY.md`).
> - **DOING:** *nothing in progress.* The build is feature-complete and ship-ready.
> - **WANT (the only thing left — user's call, outward-facing):** actual **cutover** — provision the parked Hetzner box → run the deploy kit → one-time SQLite→PG data migration (`dual_ci`) → point users at React. **Do NOT deploy or migrate without the user's explicit go-ahead + target details.** Optional (unstarted) improvement backlog: `docs/NEW_STACK_HANDOFF.md` §5 (Alembic migrations · CI frontend build · React error boundary · rate-limit public auth · site-scoped reads · DB indexes · …).
>   - **Deploy (Hetzner) stays parked** until the user asks.
>
> **DONE this session (all committed + pushed, newest first):**
> - **🛠️ Tier-1 review quick wins (4 small real gaps).** After an architecture review the user green-lit these. (1) **Alembic migrations** — `backend/alembic/` + autogenerated baseline (64 tables); cutover flow = `dual_ci` load → `alembic stamp head` → future changes via autogenerate (`alembic check` proves dual_ci's schema == baseline). (2) **CI frontend build** — `frontend-build` job in `postgres-dual-ci.yml`. (3) **React error boundary** — `ErrorBoundary` wraps `<App/>` (page crash → recoverable Result, verified by a throw test). (4) **Rate-limit public auth** — `ratelimit.py` (login/2fa 10/min, register 5/min per IP via nginx `X-Real-IP`). `service_tests` **54/0**; all gates green. `backend/{alembic/**,alembic.ini,api/ratelimit.py,api/auth.py}`, `frontend/src/{components/ErrorBoundary.tsx,main.tsx}`, `.github/workflows/postgres-dual-ci.yml`.
> - **🚢 Deploy kit (turnkey, NOT deployed).** User picked cutover option #1 — prepare everything, zero prod contact. New-stack `deploy/` (separate from the Streamlit repo-root compose): `docker-compose.prod.yml` (db postgres:16 · api FastAPI internal · web nginx :80/:443 SPA+`/api`-proxy+TLS · certbot auto-renew), `Dockerfile.api` (uvicorn 4 workers, GI_ENV=production), `Dockerfile.web` (Vite build→nginx), `nginx.conf` (SPA fallback + `/api/`→api:8000/ prefix-strip + TLS + ACME), `init-letsencrypt.sh`, `.env.example`. Full runbook **`docs/DEPLOY.md`** (provision→secrets→TLS→up→**one-time SQLite→PG `dual_ci` migration [wipes target, pre-go-live only]**→verify→cutover→ops/backups→rollback). Small code: `config.py` CORS now env-driven (`CORS_ORIGINS`); `.dockerignore` += node_modules/dist. **Verified locally** (no Docker on either machine): compose YAML valid, script `bash -n` clean, `service_tests` 52/0, `bug_check` 599/0, crawler 21/21, build green. **Cutover now needs only: provision the box + run the kit + one-time data migration + point users at it.** `deploy/**`, `docs/DEPLOY.md`, `backend/api/config.py`.
> - **🔔 More notification events — staging→HOD + approve/reject→submitter.** Backend-only (the bell renders any notification). Wired at the router layer to avoid a circular import (ledger↔notifications): `entry.py` fires `entry_staged`→hod@site after all 4 stages; `hod.py` fires `entry_approved`/`entry_rejected`→the submitter on approve/reject. Submitter resolved per kind via `_SUBMITTER_COL` (issues=`Issued_By`, returns/adjustments=`submitted_by`, **receipts=None** — `stage_receipt` stores no submitter, gracefully skipped). **Verified live:** stage receipt→HOD notified; approve issue→submitter "approved" (success); reject return→submitter "rejected: reason" (warning). `service_tests` +2 (`_submitter` resolution incl. receipts→None) → **52/0**. `bug_check` 599/0. **Next: peripheral tabs · report scheduler · cutover.** `backend/api/{entry,hod,service_tests}.py`.
> - **🙋 User registration + approval — self-service onboarding.** The new app only *logged in* existing users; now a public **Request Access** → `pending_users` → admin approval flow exists. `POST /auth/register` (public, `auth.py`): bcrypt→pending_users; guards username-exists-409, dup-pending-409 (revives a rejected row), short-pw-422, **requested role ≠ admin** (no self-elevation). Admin (`admin.py`, level 4): `GET /admin/pending-users` (no secret), `POST .../{id}/approve` (→ real user, role/warehouse overridable, carries the bcrypt hash, audits APPROVE_USER), `POST .../{id}/reject`. **+4 endpoints (89 paths).** **Verified live + in-browser:** register form → admin **Access Requests** approve → `ui_reg` logs in as store_keeper. Fixed a React `<Form>`-reuse bug (distinct `key`s so the register form's initialValues apply). `service_tests` +6 → **50/0**. Frontend: LoginPage register mode, `PendingUsersPage` + **Admin→Access Requests** nav. Test users PG-only → reset (8 users). `bug_check` 599/0. **Only user-registration remained of the old auth gaps; done.** `backend/api/{auth,admin,service_tests}.py`, `frontend/src/pages/{LoginPage,PendingUsersPage}.tsx`, `frontend/src/{App,components/AppLayout,api/hooks}.tsx`.
...
> - **🤖 Migration status (FINAL 2026-07-18):** the portability program was superseded — the **new FastAPI+React+PostgreSQL stack is FEATURE-COMPLETE** (see `docs/PROJECT_STATUS.md`); the legacy app is frozen under `legacy/` on SQLite until switch-off. Gates: service_tests 750/0 · Playwright 39/39 · bug_check 599/0. Only the Hetzner deployment remains (`tools/migration/README.md`). *(Historical 2026-07-01 status: interactive build, routine paused, Phase 3A at 10/~55 — see `docs/POSTGRES_MIGRATION.md` §7/§8.)*
...
>    - **🤖 Migration status (FINAL 2026-07-18):** SUPERSEDED — `FRONTEND_GO` was approved 2026-07-04 and the FastAPI+React+PostgreSQL stack is now **FEATURE-COMPLETE** (gates 750/0 · 39/39 · 599/0; see `docs/PROJECT_STATUS.md`). This coordination box is historical; only the Hetzner deployment remains.
...
- **`sme_recipe`** — lining-system recipe master (global, not site-scoped). Legacy SQLite: `UNIQUE(Lining_System_Code, Material_Code)`. **New-stack PG (2026-07-18, alembic b3f2a9c47d18): `UNIQUE(Lining_System_Code, Material_Code, SAP_Code)`** — PU systems carry Comp-A/B/C/D lines sharing one Material_Code, distinguished only by variant SAPs. Carries `For_1_SQM` and `Nature`.
