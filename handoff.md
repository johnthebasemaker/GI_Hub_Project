...
> - **🤖 Migration status (2026-07-01, interactive):** Phase 3 sub-phase A **in progress** — routing `init_db()`'s `PRAGMA table_info` self-heal sites through `column_exists()`. **10/~55 sites** done (1 Phase-2 + 6 routine increment 1 + 3 interactive increment 2: `pending_receipts.rejection_reason` + `receipts`/`pending_receipts` DN/PO/Warehouse trace loops). `PRAGMA table_info` repo-wide 88→85. SQLite still default; bug_check unchanged from clean baseline (this machine: 560/20, the 20 = pre-existing missing-deps env failures, verified zero regressions by stash-compare). Next: `docs/POSTGRES_MIGRATION.md` §7 "Next action" (easy sites now largely exhausted — remaining is sensitive/multi-column triage). Still SQLite in prod.
...
>
> **PENDING / NEXT:**
> 1. **PostgreSQL Phase 3+ — NOW RUNNING VIA AN AUTONOMOUS ROUTINE.** A scheduled cloud routine (**`GI-Hub autonomous`**, Mon/Wed/Fri, laptop-off) advances the Postgres migration one bounded increment per run and opens a **PR on a `claude/*` branch** (never merges to `main`; the user reviews + merges). **Increment 1 already merged** (`c8dd848`: 6 `PRAGMA table_info` self-heal sites → `column_exists()`).
...
>    - **🤖 Migration status:** Phase 3 sub-phase A in progress — 10/~55 `init_db()` `PRAGMA` self-heal sites routed through `column_exists()` (+3 interactive increment 2: `pending_receipts.rejection_reason` + `receipts`/`pending_receipts` DN/PO/Warehouse trace loops). SQLite still default; zero regressions vs baseline. Next action: ledger §7.
...
15. 🔶 **Per-site Unit_Cost — FOUNDATION DONE (additive), rest incremental.** New `inventory_site_costs` table + `get_effective_unit_cost()` (COALESCE site→global→0) + `set_site_unit_cost()`/`get_site_unit_costs()` + Admin → Settings "💲 Per-Site Unit Cost" editor. **Migrated:** `get_value_by_site()`, `get_total_inventory_value()` (company total now sums per-site), `get_inventory_valuation(site_id=…)`. **Zero behaviour change until an override is set.** +1 `bug_check` regression. **Still on global cost (migrate incrementally, each with a test):** movement-value view (`database.py` ~2889/2899), receipts-value report (~6701), monthly SAP-value report (~6784), and the per-SAP company rollup branch of `get_inventory_valuation(None)`. `database.py`, `pages_internal/admin_portal.py`.
