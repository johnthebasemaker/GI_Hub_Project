...
**Last update:** 2026-07-02 — **FastAPI REST backend v1 (async, PostgreSQL) — runnable & viewable locally.** The Postgres data-layer migration is now the live foundation for a decoupled REST API. Streamlit/SQLite untouched. Tests: **599 bug_check / 0 failed · UI crawler 21/21**. Per-item detail in the commit messages (`git log --oneline`).
