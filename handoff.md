...
>    - **🤖 Migration status (2026-07-01, routine increment 3):** Phase 3 sub-phase A in progress — 16/~55 `init_db()` `PRAGMA` self-heal sites routed through `column_exists()` (+6 this run: `Lot_Number` self-heal across `receipts`/`consumption`/`pending_issues`/`pending_receipts` + `FEFO_Override` self-heal across `pending_issues`/`consumption` — pure traceability/audit-reason columns, not identity math). `PRAGMA table_info` repo-wide 85→83. SQLite still default; zero regressions vs baseline (Python 3.11: 576/4 identical to stash-verified baseline; Python 3.12: 579/580 bug_check + 21/21 crawler, matching the recorded baseline exactly). Next action: ledger §7 — `inventory`/`receipts.Unit_Cost` and the multi-column-reuse blocks (`pr_master`, extended-column loops) still need individual triage.
