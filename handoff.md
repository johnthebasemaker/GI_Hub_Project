...
**Last update:** 2026-07-02 — **FastAPI REST backend v1 + v2 (async, PostgreSQL).** v1 = read API + docs; v2 = parity-tested derived stock endpoints + master-data writes. Next: the React frontend. Streamlit/SQLite untouched. Tests: **599 bug_check / 0 failed · UI crawler 21/21 · derived-view parity PASS**. Per-item detail in the commit messages (`git log --oneline`).
