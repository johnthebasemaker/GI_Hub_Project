...
> - **🤖 Migration status (2026-07-01, routine):** Phase 3 sub-phase A **in progress** — routing `init_db()`'s `PRAGMA table_info` self-heal sites through `column_exists()`. 7/~55 sites done (1 Phase-2 + 6 this run: `returnable_items`, `pending_users`, `whatsapp_queue`, `employees`, `supervisor_material_request_items`). SQLite still default; 579/580 bug_check + 21/21 crawler green (baseline unchanged). Next: `docs/POSTGRES_MIGRATION.md` §7 "Next action". Still SQLite in prod.
...
>    - **🤖 Migration status:** Phase 3 sub-phase A in progress — 7/~55 `init_db()` `PRAGMA` self-heal sites routed through `column_exists()`. SQLite still default; 580/0 · 21/21. Next action: ledger §7.
