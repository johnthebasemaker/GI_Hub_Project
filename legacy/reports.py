...
def generate_pr_pdf(
    ...
    generated_by: str,
    *,
    po_map: dict[int, str] | None = None,
) -> bytes:
    """
    Generates an official PDF status report for a Purchase Request,
    including immutable metadata (Who and When).

    Round 16 — added two columns: **PO #** (comma-joined per PR line, fed
    from `po_map={pr_line_id: 'PO-001, PO-002'}` so multi-PO PRs render
    cleanly) and **UoM** (from pr_master.UOM). Column widths redistributed
    to keep the 8-column table on A4 portrait inside the existing margins.
    `po_map` defaults to None — back-compat with the pre-Round-16 caller;
    the PO column simply renders blank in that case.
    """
    ...
    # Round 16 — 8 columns share the 190 mm strip:
    # Material Code 25 / Description 55 / PO # 25 / UoM 12 /
    # Req. Qty 18 / Received 18 / Pending 18 / Status 19  = 190 mm
    col_widths = [25, 55, 25, 12, 18, 18, 18, 19]
    headers = ["Material Code", "Description", "PO #", "UoM",
               "Req. Qty", "Received", "Pending", "Status"]
    for i, h in enumerate(headers):
        is_last = (i == len(headers) - 1)
        pdf.cell(
            col_widths[i], 8, h, border=1,
            # Quantitative + status columns centre-aligned; text columns left.
            align="C" if i >= 3 else "L", fill=True,
    ...
    for _, row in pr_df.iterrows():
        line_id  = int(row.get("id", 0) or 0)
        mat_code = str(row.get("Material_Code", "N/A"))
        desc     = str(row.get("Material_Name", "Unknown Material"))[:30]
        # Round 16 — PO # column. Truncate long lists with '…' so the cell
        # never overflows its width budget (25 mm at 8 pt fits ~14 chars).
        po_text  = po_map.get(line_id, "")
        if len(po_text) > 14:
            po_text = po_text[:13] + "…"
        uom      = str(row.get("UOM", "") or "")[:6]
        ...
        pdf.cell(col_widths[2], 8, po_text, border=1, align="C")
        pdf.cell(col_widths[3], 8, uom, border=1, align="C")
        pdf.cell(col_widths[4], 8, f"{req:g}", border=1, align="C")
        pdf.cell(col_widths[5], 8, f"{recv:g}", border=1, align="C")
        pdf.cell(col_widths[6], 8, f"{pend_disp:g}", border=1, align="C")
        pdf.cell(col_widths[7], 8, status, border=1, align="C",
